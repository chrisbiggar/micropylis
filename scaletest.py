...
class BgGroup(Group):
    ...
    def set_state(self):
        ...
        self._scaleOffX = ((self.tilesView.width/2) * (1-self._zoom))
        self._scaleOffY = (self.tilesView.height/2) * (1-self._zoom)
...
class ScaleTestApp(pyglet.tilesView.Window):
    ...
    def on_key_release(self, symbol, modifiers):
        if symbol == key.EQUAL:
            self.changeZoom(increment=0.05)
        if symbol == key.MINUS:
            self.changeZoom(increment=-0.05)
        if symbol == key.P:
    ...
    def on_mouse_release(self, x, y, button, modifiers):
        ...
        if button == pyglet.tilesView.mouse.RIGHT:
            ...
            x = (x - self.group._scaleOffX) / self.group._zoom + self.scrollX
            #print self.scrollX
            print y,self.group._scaleOffY,self.scrollY,self.group._zoom
            y = self.height - (y - self.group._scaleOffY) / self.group._zoom + self.scrollY 
            print x,y
